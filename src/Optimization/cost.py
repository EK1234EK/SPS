...
def cost_function(dof_vec):
    ...
    SMA_track = list(np.array(dof_vec[0:5]) * normalization_factors["SMA"])
    ECC_track = list(np.array(dof_vec[5:10]) * normalization_factors["ECC"])
    INC_track = list(np.array(dof_vec[10:15]) * normalization_factors["INC"])
    ...
    for i, sc in enumerate(sw_1.list_of_spacecraft):
        ...
        print("Minimum cost: ", round(min(t_total) * 10**(-7), 2), " at integration point ", argmin(t_total))
        print("SMA: ", dof_vec[0:5])
        print("ECC: ", dof_vec[5:10])
        print("INC: ", dof_vec[10:15])
